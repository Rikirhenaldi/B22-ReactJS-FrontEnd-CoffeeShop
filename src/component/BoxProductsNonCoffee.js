...
class BoxProductsNonCoffee extends React.Component{
    state = {
        data: []
    }
    componentDidMount(){
    ...
    getData = async() => {
        const {data} = await axios.get('http://localhost:8080/products/category/2')
            ...
            <div className="flex-1 space-y-12 border-t-2 ">
                <div className="navigasi mx-28 text-xl text-gray-400 mt-8">
                    <ul className="flex flex-row justify-between items-center text-center">
                        ...
                        <li><Link to="/Product/Category/2" className="text-yellow-800 font-bold underline">Non Coffee</Link></li>
...
export default BoxProductsNonCoffee